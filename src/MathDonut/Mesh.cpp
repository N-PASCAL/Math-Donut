...
Mesh::Mesh(Settings const& settings) : m_resolution(settings.GetMeshResolution())
{
}
...
void Mesh::_GenerateSector(float radius, float angle)
{
    ...
    for (int i = 0; i < m_resolution; i++)
    {
        ...
        for (int j = 0; j < m_resolution; j++)
        {
            ...
            m_vertices[m_resolution * i + j].x = r * cos(theta);
            m_vertices[m_resolution * i + j].y = r * sin(theta);
