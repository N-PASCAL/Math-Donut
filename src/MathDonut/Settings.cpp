...
void Settings::_ParseArguments(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) //i starts at 1 because command line arguments start with argv[1] (argv[0] is the name of the exe)
    {
        ...
        if (arg == "-w" && i + 1 < argc)
            m_screenWidth = std::atoi(argv[++i]);
        else if (arg == "-h" && i + 1 < argc)
            m_screenHeight = std::atoi(argv[++i]);
        else if (arg == "-b" && i + 1 < argc)
            m_screenBackground = argv[++i][0];
        else if (arg == "-p" && i + 1 < argc)
            m_screenMeshProjection = argv[++i][0];
        else if (arg == "-s" && i + 1 < argc)
            m_screenPosition = std::atof(argv[++i]);
        else if (arg == "-r" && i + 1 < argc)
            m_meshResolution = std::atoi(argv[++i]);
        else if (arg == "-m" && i + 1 < argc)
            m_meshPosition = std::atof(argv[++i]);
        else if (arg == "-x" && i + 1 < argc)
            m_meshRotationXPerFrame = std::atof(argv[++i]);
        else if (arg == "-y" && i + 1 < argc)
            m_meshRotationYPerFrame = std::atof(argv[++i]);
        else if (arg == "-z" && i + 1 < argc)
            m_meshRotationZPerFrame = std::atof(argv[++i]);
        else if (arg == "-f" && i + 1 < argc)
            m_frameDuration = std::atoi(argv[++i]);
    }
}